...
filament_height = 13.4; //distance from motor to center of filament.  Adjust to suit.

//these are for the drive gear, aka hobbed pulley
gear_dia = 9.25;  //13 for Mark 7, 9 for Mark 8
gear_rad = gear_dia/2+slop;

//the eff gear rad is the radius of the hobbed part of your gear, 
eff_gear_rad = 7.2/2; //11/2 for mark 7, 7/2 for Mark 8
...
//extruder(groovemount_screw, none, bondtech);
...
module body(solid = 0, type=0, mount=0, idler=0){
	...
	//hollow bits
        ...
        translate([0,0,filament_height-5]){
            cylinder(r=gear_rad+gear_screw_clearance+slop*2, h=2.05, center=true);
            for(i=[0,1]) mirror([0,0,i]) translate([0,0,1.5]) cylinder(r1=gear_rad+gear_screw_clearance+slop*2, r2=gear_rad+slop, h=1.05, center=true);
        }
        ...
        if(idler == bondtech){
			idler(idler_dia = 8, idler_thick = 6, idler_flat_rad = 3.5, idler_nut_rad = m3_nut_rad, idler_nut_height = m3_nut_height, idler_bolt_rad = m3_rad, slit_w=.5);
		}

		//cone to guide/unguide filament
		translate([filament_offset,-1+.5,filament_height]) for(i=[0,1]) mirror([0,i,0]) translate([0,-wall-1+2.5-.5,0]) rotate([90,0,0]) rotate([0,0,180/8]) cylinder(r1=filament_rad*2.5, r2=filament_rad, h=3, $fn=8);
...
module idler(idler_dia = 16, idler_thick = 6, idler_flat_rad = 4, idler_nut_rad = m5_nut_rad, idler_nut_height = m5_nut_height, idler_bolt_rad = m5_rad, slit_w=1.5){
	...
	translate([eff_gear_rad+idler_rad+idler_offset,0,0]){
		...
		difference(){
			hull(){
				...
				translate([-idler_offset/2,0,filament_height+1]) scale([1,.9,1]) cylinder(r=idler_rad+.25, h=idler_thick+4, center=true);
			}
