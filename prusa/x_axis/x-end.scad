...
!translate([-50, 50, 0]) mirror([0, 0, 0]) translate([50, 0, 0])
    x_end_motor(spring=11.5);

m5_nut_height=5;

m8_nut_diameter_horizontal = 15;
m8_nut_height = 5;
...
module x_end_motor(spring=0){
    ...
    mirror([0, 1, 0]) {
        ...
        translate([0, -z_delta - 2, 0]) mirror([1,0,0]) translate([28.2, 0, 0]) difference(){
            union(){
                intersection() {
    ...
    difference(){
        union(){
            ...
            translate([0, 0, 4 - bushing_xy[0]]) {
                ...
                translate([z_delta, rod_inset, 0]) render(convexity = 5) linear(bushing_z, bearing_height);
                // Nut trap
                translate([-2+2.16, rod_sep+rod_inset, nut_trap_height/2]) cube_fillet([20, 20, nut_trap_height], center = true, vertical=[8, 0, 0, 5]);
                //}
            }
        }
        // here are bushings/bearings
        translate([z_delta, rod_inset, 4 - bushing_xy[0]]) linear_negative(bushing_z, bearing_height);

        // belt hole
        translate([-14 - xy_delta / 2, 22 - 9 + offset, 30]) cube_fillet([max(idler_width + 2, 11), 55+10, 27], center = true, vertical=0, top=[0, 1, 0, 1], bottom=[0, 1, 0, 1], $fn=4);

        //smooth rods
        translate([-10 - bushing_xy[0], offset, 0]) {
            if(thru == true){
                translate([0, -11, 6]) rotate([-90, 0, 0]) pushfit_rod(bushing_xy[0] * 2 + 0.3, 65);
                translate([0, -11, xaxis_rod_distance+6]) rotate([-90, 0, 0]) pushfit_rod(bushing_xy[0] * 2 + 0.3, 65);
            } else {
        ...
        translate([0, 0, 0]) {  // m8 nut insert
            translate([0, rod_sep+rod_inset, 0]) rotate([0, 0, 10]){
                //rod
                translate([0, 0, -2]) cylinder(h=(4.1 + nut_trap_height), r=4.5, $fn=32);
                //nut
                translate([0, 0, nut_trap_height-m8_nut_height]) rotate([0,0,-10]) rotate([0,0,30]) cylinder(r1=m8_nut_diameter_horizontal/2+.1, r2=m8_nut_diameter_horizontal/2+.1+.3, h=m8_nut_height+.1, $fn=6);
                translate([0, 0, nut_trap_height]) rotate([0,0,-10]) rotate([0,0,30]) cylinder(r2=m8_nut_diameter_horizontal/3, r1=m8_nut_diameter_horizontal/2+.1+.3, h=m8_nut_height+.1, $fn=6);
                
                //spring under nut
                translate([0, 0, -2]) rotate([0,0,-10]) difference(){
                    rotate([0,0,30]) cylinder(r1=m8_nut_diameter_horizontal/2+.2, r2=m8_nut_diameter_horizontal/2+.2, h=nut_trap_height, $fn=6);
                    translate([0,7+2.8,nut_trap_height-m8_nut_height]) cube([10,10,2], center=true);
                    translate([0,-7-2.8,nut_trap_height-m8_nut_height]) cube([10,10,2], center=true);
                    translate([7+2.8,0,nut_trap_height-m8_nut_height+.25]) cube([10,10,2-.5], center=true);
                    translate([-7-2.8,0,nut_trap_height-m8_nut_height+.25]) cube([10,10,2-.5], center=true);
                }
