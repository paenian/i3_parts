...
module linear_bearing_negative_single(conf_b=bushing_xy, h=0, shadow=false){
    ...
    //zip tie slot
    if(shadow == false){
        if(bushing_clamp==bushing_zip){
            translate([0,0,adjust_bushing_len(conf_b, h)/2 + 3 * layer_height]) difference(){
                cylinder(r=bushing_outer_radius(conf_b)-.5+zip_t, h=zip_h, center=true, $fn=60);
                cylinder(r=bushing_outer_radius(conf_b)-.5, h=zip_h, center=true, $fn=60);
            }
        }
    }
...
module linear_bearing(conf_b=bushing_xy, h=0){
    difference() {
        union() {
            ...
            if(bushing_clamp==bushing_screw){
                intersection(){
